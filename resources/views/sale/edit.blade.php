            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-primary">
                        ...
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form role="form" action="{{ route('sale.updateaction', $row->id_penjualan) }}" method="POST">
                            @method('PUT')
                            @csrf
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="konsumen" class="form-control" id="konsumen"
                                        placeholder="Enter Product Name" value="{{ $row->nama_konsumen }}" required>
                                </div>
                                <div class="form-group">
                                    <label for="product">Produk</label>
                                    <select name="product" id="product" class="form-control">
                                        @foreach ($product as $r)
                                            <option value="{{ $r->id_produk }}"
                                                {{ $r->id_produk == $row->id_produk ? 'selected' : '' }}>
                                                {{ $r->kode_produk }} -
